...
from percentage_ide.core.file_handling import file_manager
...
PERCENTAGE_EXECUTABLE = os.path.realpath(sys.argv[0])
...
HOME_PERCENTAGE_PATH = os.path.join(HOME_PATH, ".percentage_ide")

PERCENTAGE_KNOWLEDGE_PATH = os.path.join(HOME_PERCENTAGE_PATH, 'knowledge')

SETTINGS_PATH = os.path.join(HOME_PERCENTAGE_PATH, 'percentage_settings.ini')

DATA_SETTINGS_PATH = os.path.join(HOME_PERCENTAGE_PATH, 'data_settings.ini')

EXTENSIONS_PATH = os.path.join(HOME_PERCENTAGE_PATH, "extensions")

SYNTAX_FILES = os.path.join(PRJ_PATH, "gui", "editor", "syntaxes")

PLUGINS = os.path.join(HOME_PERCENTAGE_PATH, "extensions", "plugins")

BACKUP_FILES = os.path.join(HOME_PERCENTAGE_PATH, "backups")
...
PERCENTAGE_THEMES_DOWNLOAD = os.path.join(EXTENSIONS_PATH, "theme")

PERCENTAGE_THEMES = os.path.join(PRJ_PATH, "extensions", "theme")

# PERCENTAGE_THEME = os.path.join(PRJ_PATH, "extensions", "theme", "percentage_dark.qss")
PERCENTAGE_QSS = os.path.join(PRJ_PATH, "extensions", "theme", "qss")

LOG_FILE_PATH = os.path.join(HOME_PERCENTAGE_PATH, 'percentage_ide.log')
...
BUGS_PAGE = "https://github.com/percentage-ide/percentage-ide/issues"

PLUGINS_DOC = "http://percentage-ide.readthedocs.org/en/latest/"

UPDATES_URL = 'http://percentage-ide.org/updates'

SCHEMES_URL = 'http://percentage-ide.org/schemes/api/'

LANGUAGES_URL = 'http://percentage-ide.org/plugins/languages'

PLUGINS_WEB = 'http://percentage-ide.org/plugins/api/official'

PLUGINS_COMMUNITY = 'http://percentage-ide.org/plugins/api/community'
...
def create_home_dir_structure():
    """
    Create the necesary directories structure for PERCENTAGE-IDE
    """
    for directory in (HOME_PERCENTAGE_PATH, EXTENSIONS_PATH, PLUGINS, EDITOR_SKINS,
                      LANGS, PERCENTAGE_THEMES_DOWNLOAD, PERCENTAGE_KNOWLEDGE_PATH,
                      BACKUP_FILES):
...
def load_theme(name="Dark"):
    themes = {}
    for theme_dir in (PERCENTAGE_THEMES, PERCENTAGE_THEMES_DOWNLOAD):
        files = file_manager.get_files_from_folder(theme_dir, ".percentagetheme")
    ...
    percentage_theme = themes[name]
    global QML_COLORS
    QML_COLORS = percentage_theme["qml"]
    return percentage_theme
