...
logger = PercentageLogger('percentage_ide.core.file_handling.filesystem_notifications.linux')
DEBUG = logger.debug
...
#from percentage_ide.core.file_handling.filesystem_notifications.base_watcher import ADDED, \
#                                            DELETED, REMOVE, RENAME, MODIFIED

mask = IN_CREATE | IN_DELETE | IN_DELETE_SELF | IN_MODIFY


class PercentageProcessEvent(ProcessEvent):
...
class QNotifier(QThread):
    def __init__(self, wm, processor):
        ...
        self.notifier = Notifier(wm,
                            PercentageProcessEvent(self.event_queue.append))
