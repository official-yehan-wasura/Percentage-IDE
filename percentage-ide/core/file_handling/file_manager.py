...
class PercentageIOException(Exception):
    """
...
class PercentageNoFileNameException(Exception):
    """
...
class PercentageFileExistsException(Exception):
    """
...
def create_init_file(folderName):
    ...
    if not os.path.isdir(folderName):
        raise PercentageIOException("The destination folder does not exist")
    name = os.path.join(folderName, '__init__.py')
    if file_exists(name):
        raise PercentageFileExistsException(name)
    f = open(name, 'w')
    f.flush()
    f.close()
...
def create_folder(folderName, add_init_file=True):
    ...
    if os.path.exists(folderName):
        raise PercentageIOException("The folder already exist")
...
def read_file_content(fileName):
    ...
    try:
        ...
        raise PercentageIOException(reason)
    except Exception:
        raise
    return content
...
def store_file_content(fileName, content, addExtension=True, newFile=False):
    ...
    if newFile and file_exists(fileName):
        raise PercentageFileExistsException(fileName)
    try:
        ...
        if not f.open(flags):
            raise PercentageIOException(f.errorString())
...
def open_project_with_extensions(path, extensions):
    ...
    if not os.path.exists(path):
        raise PercentageIOException("The folder does not exist")
...
def rename_file(old, new):
    ...
    if os.path.isfile(old):
        if file_exists(new):
            raise PercentageFileExistsException(new)
        os.rename(old, new)
        return new
    return ''
...
def check_for_external_modification(fileName, old_mtime):
    """Check if the file was modified outside percentage."""
