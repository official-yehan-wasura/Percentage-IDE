...
from percentage_ide.tools.utils import SignalFlowControl
from .file_manager import PercentageIOException, PercentageNoFileNameException, \
    get_file_encoding, get_basename, get_file_extension

from percentage_ide.tools.logger import PercentageLogger
logger = PercentageLogger('percentage_ide.core.file_handling.nfile')
DEBUG = logger.debug
...
class NFile(QObject):
    ...
    def save(self, content, path=None):
        ...
        if not save_path:
            raise PercentageNoFileNameException("I am asked to write a "
    ...
    def read(self, path=None):
        ...
        if not self._file_path:
            raise PercentageNoFileNameException("I am asked to read a file "
