...
logger = PercentageLogger('percentage_ide.core.plugin_manager')
...
class ServiceLocator(object):
    ...
    '''
    Hold the services and allows the interaction between PERCENTAGE-IDE and plugins
    '''
...
class MyPluginExample(Plugin):
    ...
    def initialize(self):
        #Configure the plugin using the PERCENTAGE-IDE API!!!
...
#Extension of the PERCENTAGE-IDE plugin
...
def _availables_plugins(url):
    """
    Return the availables plugins from an url in PERCENTAGE-IDE web page
    """
    try:
...
def available_oficial_plugins():
    '''
    Returns a dict with OFICIAL availables plugins in PERCENTAGE-IDE web page
    '''
...
def available_community_plugins():
    '''
    Returns a dict with COMMUNITY availables plugins in PERCENTAGE-IDE web page
    '''
...
def update_local_plugin_descriptor(plugins):
    ...
    downloaded with PERCENTAGE-IDE
