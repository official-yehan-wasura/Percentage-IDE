...
logger = PercentageLogger('percentage_ide.core.encapsulated_env.nenvironement')
...
from percentage_ide.resources import HOME_PERCENTAGE_PATH
PERCENTAGE_ENV_NAME = "venv"
PERCENTAGE_ENV = os.path.join(HOME_PERCENTAGE_PATH, PERCENTAGE_ENV_NAME)
PERCENTAGE_ENV_BIN = os.path.join(PERCENTAGE_ENV, "bin")

if not os.path.isdir(PERCENTAGE_ENV):
    create_environment(PERCENTAGE_ENV)
if not os.path.isdir(PERCENTAGE_ENV_BIN):
    PERCENTAGE_ENV_BIN = os.path.join(PERCENTAGE_ENV, "Scripts")


PERCENTAGE_ENV_ACTIVATE = os.path.join(PERCENTAGE_ENV_BIN, "activate_this.py")


exec(compile(open(PERCENTAGE_ENV_ACTIVATE).read(), PERCENTAGE_ENV_ACTIVATE, 'exec'),
     dict(__file__=PERCENTAGE_ENV_ACTIVATE))
...
PLUGIN_QUERY = {"keywords": "percentage_ide plugin"}
